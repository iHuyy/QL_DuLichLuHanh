...
create table dichvu (
   ...
   mota         nvarchar2(300),
   maloaidichvu number,
   constraint fk_dv_loaidv foreign key ( maloaidichvu )
      references loaidichvu ( maloaidichvu )
);
...
create table nhacungcap_dichvu (
   ...
   constraint pk_nccdv primary key ( manhacungcap,
                                     madichvu ),
   constraint fk_nccdv_ncc foreign key ( manhacungcap )
      references nhacungcap ( manhacungcap ),
   constraint fk_nccdv_dv foreign key ( madichvu )
...
create table tour (
   ...
   trangthai   nvarchar2(50) default 'Hoạt động'   CHECK (TRANGTHAI IN ('Hoạt động','Đang diễn ra','Hoàn thành','Tạm ngưng','Đã kết thúc','Đã hủy','Ẩn'));
...
create table chitiettour (
   ...
   madichvu number,
   soluong  number,
   constraint pk_cttour primary key ( matour,
                                      madichvu ),
   constraint fk_cttour_tour foreign key ( matour )
      references tour ( matour ),
   constraint fk_cttour_dv foreign key ( madichvu )
      references dichvu ( madichvu )
);
