...
namespace DuLich.Controllers
{
    public class CustomerController : BaseController
    {
        ...
        public async Task<IActionResult> Login(LoginModel model)
        {
            ...
            ModelState.AddModelError(string.Empty, "Tên dang nh?p và m?t kh?u chua chính xác");
            return View(model);
        }
        ...
        public async Task<IActionResult> MyTour()
        {
            ...
            foreach (var booking in bookedTours)
            {
                ...
                if (booking.TrangThaiDat == "Ðã xác nh?n" && tour.ThoiGian > DateTime.Now)
                {
                    bookingStatusChar = "y"; // Upcoming
                }
                else if (booking.TrangThaiDat == "Ðã xác nh?n" && tour.ThoiGian <= DateTime.Now)
                {
                    bookingStatusChar = "f"; // Finished
                }
                else if (booking.TrangThaiDat == "Ðã h?y")
                {
                ...
                model.MyTours.Add(new MyTourItem
                {
                    ...
                    Images = imageIds.Select(id => $"/api/image/{id}").ToList(),
                    Rating = rating,
                    IsPaid = booking.HoaDon?.TrangThai == "Ðã thanh toán"
                });
            }
        ...
        public async Task<IActionResult> TourDetail(int id)
        {
            ...
            {
                MaTour = tour.MaTour,
                TenTour = tour.TieuDe ?? "Chua có tên",
                MoTa = tour.MoTa,
                DiemKhoiHanh = tour.NoiKhoiHanh ?? "Chua xác d?nh",
                DiemDen = tour.NoiDen ?? tour.ThanhPho ?? "Chua xác d?nh",
            ...
            ViewBag.RelatedTours = await _context.Tours
                ...
                {
                    MaTour = t.MaTour,
                    TenTour = t.TieuDe ?? "Chua xác d?nh",
                    Gia = t.GiaNguoiLon ?? 0
                })
                .ToListAsync();
        ...
        public async Task<IActionResult> Booking([FromForm] CreateBookingViewModel model)
        {
            ...
            {
                ModelState.AddModelError(string.Empty, "S? lu?ng ngu?i d?t vu?t quá s? ch? còn tr?ng");
                return View(model);
            }
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var booking = new DatTour
                    {
                        ...
                        TrangThaiDat = "Chua xác nh?n", // Logic PHP d? là 'Chua xác nh?n'
                        TrangThaiThanhToan = "Chua thanh toán",
                    ...
                    // 2. T?O HÓA ÐON (Luu tru?c d? l?y MaHoaDon và NgayXuat chu?n)
                    // Ki?m tra xem trigger có t? t?o hóa don không, n?u chua thì t?o th? công
                    ...
                    // 3. T?O PAYLOAD JSON (Ð?NG B? V?I PHP/FLUTTER)
                    // C?u trúc này kh?p hoàn toàn v?i file create_booking.php b?n dã g?i
                    var payloadObj = new
                    {
                        maHoaDon = hoaDon.MaHoaDon,
                        maDatTour = booking.MaDatTour,
                        maKhachHang = booking.MaKhachHang,
                        soTien = (double)(hoaDon.SoTien ?? 0), // Ép ki?u double cho gi?ng JSON number
                        ngayXuat = hoaDon.NgayXuat?.ToString("yyyy-MM-dd HH:mm:ss"), // Ð?nh d?ng ngày gi?ng Oracle TO_CHAR
                    ...
                    // 4. KÝ S?
                    // Ký chu?i JSON v?a t?o
                    string signature = _rsaService.Sign(payloadJson);

                    // 5. C?P NH?T L?I VÀO DATABASE
                    hoaDon.Payload = payloadJson; // Luu JSON g?c vào c?t Payload
                    hoaDon.ChuKySo = signature;   // Luu ch? ký
        ...
        public async Task<IActionResult> TourBooked(int bookingId)
        {
            ...
            var myTourItem = new MyTourItem
            {
                ...
                BookingStatus = booking.TrangThaiDat == "Ðã xác nh?n" && tour.ThoiGian > DateTime.Now ? "y" :
                                booking.TrangThaiDat == "Ðã xác nh?n" && tour.ThoiGian <= DateTime.Now ? "f" :
                                booking.TrangThaiDat == "Ðã h?y" ? "c" : "b", // 'b' for pending, 'y' for upcoming, 'f' for finished, 'c' for cancelled
                Title = tour.TieuDe ?? string.Empty,
                Description = tour.MoTa ?? string.Empty,
                Destination = tour.NoiDen ?? tour.NoiKhoiHanh ?? tour.ThanhPho ?? string.Empty,
                Time = tour.ThoiGian?.ToString("yyyy-MM-dd") ?? string.Empty,
        ...
        public async Task<IActionResult> CancelBooking(int bookingId)
        {
            ...
            if (booking.TrangThaiDat != "Ðã h?y" && booking.TrangThaiDat != "?? ho?n th?nh")
            {
                booking.TrangThaiDat = "Ðã h?y";
        ...
        public async Task<IActionResult> Payment(int bookingId)
        {
            ...
            var booking = await _dbContext.DatTours
                .Include(b => b.Tour)
                .Include(b => b.HoaDon) // Entity HoaDon gi? dã có tru?ng Payload
            ...
            {
                TempData["ErrorMessage"] = "Không tìm th?y thông tin d?t tour ho?c hóa don";
                return RedirectToAction("MyTour");
            }

            // *** B?T Ð?U S?A Ð?I LOGIC KI?M TRA ***

            bool isValid = false;

            // 1. L?y Payload g?c (JSON) và Ch? ký t? Database
            string payloadJson = booking.HoaDon.Payload ?? string.Empty;
            string signature = booking.HoaDon.ChuKySo ?? string.Empty;

            // 2. Ki?m tra: Ch? verify khi có d? d? li?u
            if (!string.IsNullOrEmpty(payloadJson) && !string.IsNullOrEmpty(signature))
            {
                // G?i RSAService d? verify (Hàm này ph?i dùng SHA256 và PKCS1 nhu b?n dã s?a ? RSAService)
                isValid = _rsaService.Verify(payloadJson, signature);
            }
            else
            {
                // N?u thi?u Payload ho?c Ch? ký -> Coi nhu không h?p l? (ho?c chua ký)
                isValid = false;
            }
            ...
            var model = new InvoiceViewModel
            {
                ...
                // Gán k?t qu? ki?m tra vào dây d? View hi?n th?
                IsSignatureValid = isValid,

                // Thông tin tour & khách hàng (gi? nguyên)
        ...
        public async Task<IActionResult> ConfirmPayment(int bookingId)
        {
            ...
            if (booking.HoaDon.TrangThai != "Ðã thanh toán")
            {
                booking.TrangThaiDat = "Ðã thanh toán";
                booking.TrangThaiThanhToan = "Ðã thanh toán";
                booking.HoaDon.TrangThai = "Ðã thanh toán";
                _context.DatTours.Update(booking);
                ...
                try
                {
                    ...
                    var signerName = signer?.HoTen ?? "Ngu?i qu?n lý";
                    ...
                    TempData["SuccessMessage"] = "Thanh toán thành công! Hóa don dã du?c t?o.";
                ...
                catch (Exception ex)
                {
                    ...
                    TempData["SuccessMessage"] = "Thanh toán thành công! Nhung không th? t?o hóa don PDF.";
                }
            }
        ...
        public async Task<IActionResult> DownloadInvoicePdf(int hoaDonId)
        {
            ...
            catch (Exception ex)
            {
                ...
                return StatusCode(500, $"Không th? t?o file: {ex.Message}");
            }
        }
                ...
                private byte[] CreateInvoicePdf(HoaDon hoaDon, DatTour? booking, Tour? tour, KhachHang? customer, string signerName)
        {
            try
            {
                ...
                document.Add(new Paragraph("HOA DON DAT TOUR")
                    .SetFont(fontBold)
                    .SetFontSize(18)
                ...
                headerTable.AddCell(new Cell().Add(new Paragraph("Ma hoa don: " + hoaDon.MaHoaDon).SetFont(font)));
                headerTable.AddCell(new Cell().Add(new Paragraph("Ngay xuat: " + (hoaDon.NgayXuat?.ToString("dd/MM/yyyy HH:mm:ss") ?? string.Empty)).SetFont(font)));
                headerTable.AddCell(new Cell().Add(new Paragraph("Trang thai: " + hoaDon.TrangThai).SetFont(font)));
                headerTable.AddCell(new Cell().Add(new Paragraph(" ").SetFont(font)));
                document.Add(headerTable);
                document.Add(new Paragraph(" ").SetMarginBottom(5));

                document.Add(new Paragraph("THONG TIN KHACH HANG").SetFont(fontBold).SetFontSize(12));
                var customerTable = new Table(new float[] { 1, 1 }).UseAllAvailableWidth();
                customerTable.AddCell(new Cell().Add(new Paragraph("Ten: " + (customer?.HoTen ?? string.Empty)).SetFont(font)));
                customerTable.AddCell(new Cell().Add(new Paragraph("Email: " + (customer?.Email ?? string.Empty)).SetFont(font)));
                customerTable.AddCell(new Cell().Add(new Paragraph("Dien thoai: " + (customer?.SoDienThoai ?? string.Empty)).SetFont(font)));
                customerTable.AddCell(new Cell().Add(new Paragraph("Dia chi: " + (customer?.DiaChi ?? string.Empty)).SetFont(font)));
                document.Add(customerTable);
                document.Add(new Paragraph(" ").SetMarginBottom(5));

                document.Add(new Paragraph("CHI TIET TOUR").SetFont(fontBold).SetFontSize(12));
                var tourTable = new Table(new float[] { 1, 1 }).UseAllAvailableWidth();
                tourTable.AddCell(new Cell().Add(new Paragraph("Ten tour: " + (tour?.TieuDe ?? string.Empty)).SetFont(font)));
                tourTable.AddCell(new Cell().Add(new Paragraph("Ngay khoi hanh: " + (tour?.ThoiGian?.ToString("dd/MM/yyyy") ?? string.Empty)).SetFont(font)));
                tourTable.AddCell(new Cell().Add(new Paragraph("So nguoi lon: " + (booking?.SoNguoiLon ?? 0)).SetFont(font)));
                tourTable.AddCell(new Cell().Add(new Paragraph("So tre em: " + (booking?.SoTreEm ?? 0)).SetFont(font)));
                document.Add(tourTable);
                document.Add(new Paragraph(" ").SetMarginBottom(5));

                document.Add(new Paragraph($"TONG TIEN: {(hoaDon.SoTien ?? 0):N0} VND")
                    .SetFont(fontBold)
                    .SetFontSize(14)
                    .SetTextAlignment(TextAlignment.CENTER)
                    .SetMarginTop(5)
                ...
                try
                {
                    hashBytes = SHA256.HashData(Encoding.UTF8.GetBytes(signatureSource));
                }
                catch
                {
                ...
                var hashHex = hashBytes.Length > 0 ? BitConverter.ToString(hashBytes).Replace("-", string.Empty) : string.Empty;
                var authCode = hashHex.Length >= 12 ? hashHex.Substring(0, 12) : hashHex;

                document.Add(new Paragraph("THONG TIN XAC THUC").SetFont(fontBold).SetFontSize(12));
                var signTable = new Table(new float[] { 1, 1 }).UseAllAvailableWidth();
                signTable.AddCell(new Cell().Add(new Paragraph("Chu ky so:\n" + (hoaDon.ChuKySo ?? string.Empty)).SetFont(font)));
                signTable.AddCell(new Cell().Add(new Paragraph("Ma xac thuc: " + authCode).SetFont(font)));
                document.Add(signTable);
                document.Add(new Paragraph(" ").SetMarginBottom(5));
                document.Add(new Paragraph("Hash (SHA256): " + hashHex).SetFont(font).SetFontSize(9));
                document.Add(new Paragraph(" ").SetMarginBottom(5));

                document.Add(new Paragraph("KY DUA").SetFont(fontBold).SetFontSize(12));
                var signatureTable = new Table(new float[] { 1, 1 }).UseAllAvailableWidth();
                var cell1 = new Cell().Add(new Paragraph("Nguoi lap\n\n\n\n(ky va ghi ro ho ten)").SetFont(font));
                cell1.SetMinHeight(80);
                var cell2 = new Cell().Add(new Paragraph($"Nguoi ky: {signerName}\n\n\n").SetFont(font));
                cell2.SetMinHeight(80);
...
// Thay th? hàm GenerateInvoiceHtml cu b?ng hàm này (gi?ng h?t bên API)
        private string GenerateInvoiceHtml(HoaDon hoaDon, DatTour booking, Tour tour, KhachHang customer, string signerName)
        {
            ...
            try
            {
                ...
                // L?y 12 ký t? d?u làm mã xác th?c ng?n
            ...
            // Tr? v? chu?i HTML chu?n d?p
            return $@"
    <!DOCTYPE html>
    <html>
    <head>
        <meta charset='UTF-8'>
        <title>Hóa don #{hoaDon.MaHoaDon}</title>
        <style>
    ...
    <body>
        <div class='header'>
            <h1>HÓA ÐON ÐI?N T?</h1>
            <p>Mã hóa don: <b>#{hoaDon.MaHoaDon}</b> | Ngày xu?t: {ngayXuat}</p>
        </div>

        <div class='section-title'>THÔNG TIN KHÁCH HÀNG</div>
        <table>
            <tr><td class='label'>H? tên:</td><td>{customer.HoTen}</td></tr>
            <tr><td class='label'>Email:</td><td>{customer.Email}</td></tr>
            <tr><td class='label'>S? di?n tho?i:</td><td>{customer.SoDienThoai}</td></tr>
            <tr><td class='label'>Ð?a ch?:</td><td>{customer.DiaChi}</td></tr>
        </table>

        <div class='section-title'>CHI TI?T D?CH V?</div>
        <table>
            <tr><td class='label'>Tên Tour:</td><td><strong>{tour.TieuDe}</strong></td></tr>
            <tr><td class='label'>Mã Tour:</td><td>#{tour.MaTour}</td></tr>
            <tr><td class='label'>Kh?i hành:</td><td>{ngayDi} t?i {tour.NoiKhoiHanh}</td></tr>
            <tr><td class='label'>S? lu?ng:</td><td>{booking.SoNguoiLon} Ngu?i l?n, {booking.SoTreEm} Tr? em</td></tr>
            <tr><td class='label'>Tr?ng thái:</td><td><span class='badge'>{hoaDon.TrangThai}</span></td></tr>
        </table>

        <div class='total-box'>
            T?NG THANH TOÁN: {total:N0} VNÐ
        </div>

        <div class='signature-box'>
            <div style='margin-bottom: 5px; color: #007AFF; font-weight: bold;'>THÔNG TIN XÁC TH?C (DIGITAL SIGNATURE)</div>
            <table style='margin:0'>
                <tr><td class='label' style='width:100px'>Mã ki?m tra:</td><td><b>{authCode}</b></td></tr>
                <tr><td class='label' style='width:100px'>Hash (SHA256):</td><td style='font-family:monospace; font-size:10px'>{hashHex}</td></tr>
                <tr><td class='label' style='width:100px'>Ch? ký s?:</td><td style='font-family:monospace; font-size:10px'>{hoaDon.ChuKySo}</td></tr>
            </table>
            <p style='margin-top:10px; font-style:italic; color:#666;'>
                * Hóa don này du?c ký s? b?o m?t. Quý khách có th? s? d?ng Mã ki?m tra ho?c upload file PDF d? xác th?c tính toàn v?n c?a hóa don.
            </p>
        </div>

        <div class='footer'>
            C?m on quý khách dã s? d?ng d?ch v? c?a DuLich!<br/>
            H? th?ng qu?n lý tour du l?ch tr?c tuy?n.
        </div>
        ...
        public async Task<IActionResult> VerifyInvoice(IFormFile? invoiceFile)
        {
            try
            {
                ...
                {
                    return Json(new { success = false, message = "Vui lòng ch?n file PDF." });
                }

                // 1. L?y ID t? tên file (VD: HoaDon_123.pdf -> 123)
                var fileName = invoiceFile.FileName;
                var match = System.Text.RegularExpressions.Regex.Match(fileName, @"(\d+)");

                if (!match.Success)
                {
                    return Json(new { success = false, message = "Tên file không h?p l?. Ph?i ch?a mã hóa don (VD: HoaDon_123.pdf)" });
                }

                int maHoaDon = int.Parse(match.Value);

                // 2. Truy v?n DB l?y Payload và Ch? ký
                // Quan tr?ng: Ph?i l?y c?t Payload, vì dó là d? li?u g?c lúc ký
                var hoaDon = await _dbContext.HoaDons
                    .AsNoTracking() // Không c?n track changes
                    .FirstOrDefaultAsync(h => h.MaHoaDon == maHoaDon);

                if (hoaDon == null)
                {
                    return Json(new { success = false, message = $"Không tìm th?y hóa don #{maHoaDon} trên h? th?ng." });
                }

                if (string.IsNullOrEmpty(hoaDon.Payload) || string.IsNullOrEmpty(hoaDon.ChuKySo))
                {
                    return Json(new { success = false, message = "Hóa don này chua du?c ký s? ho?c thi?u d? li?u g?c." });
                }

                // 3. G?i RSA Service d? ki?m tra
                // Tham s? 1: Payload (JSON chu?i) l?y t? DB
                // Tham s? 2: Ch? ký (Base64) l?y t? DB
                ...
                if (isValid)
                {
                    return Json(new
                    {
                        ...
                        trangThai = hoaDon.TrangThai,
                        message = "Hóa don H?P L?. Ch? ký s? kh?p hoàn toàn v?i d? li?u g?c."
                    });
                }
                else
                {
                    return Json(new
                    {
                        ...
                        isValid = false,
                        maHoaDon = hoaDon.MaHoaDon,
                        message = "C?NH BÁO: Ch? ký s? KHÔNG KH?P! D? li?u có th? dã b? s?a d?i."
                    });
                }
            }
            catch (Exception ex)
