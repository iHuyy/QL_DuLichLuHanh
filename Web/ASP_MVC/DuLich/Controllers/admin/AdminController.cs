    ...
    public class AdminController : BaseController
    {
        ...
        public async Task<IActionResult> Audit(string tab = "TOUR", int pageTour = 1, int pageStaff = 1, int pageDatabase = 1)
        {
            ...
            try
            {
                ...
                model.BackupHistories = await _context.BackupHistories
...
WHERE rn BETWEEN :startRow AND :endRow";
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        [Route("Admin/UnlockUser/{id}")]
        public async Task<IActionResult> UnlockUser(int id)
        {
            var kh = await _context.KhachHangs.FindAsync(id);
            if (kh == null || string.IsNullOrEmpty(kh.ORACLE_USERNAME))
            {
                TempData["Error"] = "Không tìm thấy thông tin tài khoản cơ sở dữ liệu.";
                return RedirectToAction("Users");
            }

            // Gọi dịch vụ để thực hiện lệnh ALTER USER ... ACCOUNT UNLOCK
            var result = await _authService.SetAccountLockAsync(kh.ORACLE_USERNAME, false);
            ...
            if (result.success)
            {
                ...
                await _context.SaveChangesAsync();
                TempData["Success"] = $"Đã mở khóa tài khoản {kh.ORACLE_USERNAME} thành công.";
            }
            else
            {
                TempData["Error"] = "Lỗi: " + result.message;
            }
